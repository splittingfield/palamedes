...
class YearFinderPipeline[@spec(Int) T](minYear:Int, maxYear:Int, threshold:Double)(implicit ev:Element[T]) extends Pipeline[T,Option[Int],Boolean] {
  ...
  def process(elem:T) = {
    ...
    possibleInt match {
      case Some(pi) =>
        if (minYear <= pi && pi <= maxYear) {
          possibleYearCount.incrementAndGet()
        }
      case None =>
    }
    possibleInt
